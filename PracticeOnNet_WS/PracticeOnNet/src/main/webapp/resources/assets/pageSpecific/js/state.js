...
$(document).ready(function() {
	...
	$(document).on("click", "#searchFormButtonId", function(e) {
		...
		searchAndReloadGrid(stateName, "", countryId);//While searching countryId will be passed not the name
	});
...
function reloadGrid() {
	...
	window.location.replace("/admin/pvt/masters/state/");
...
function searchAndReloadGrid(stateName, countryName,countryId) {
	clearCheckBoxSelection();
	var table = $('#stateId').DataTable();
	table.ajax.url(
			"paginated?stateName=" + stateName + "&countryName=" +countryName+ "&countryId="+countryId+"").load();
...
function GetSelectedRowID() {
	...
	$(".chkIndvRow").each(function() {
		if ($(this).is(':checked')) {
			...
			checkedRowNames.push(selectedRows[0].countryName +" => "+selectedRows[0].stateName + "</br>");
	...
	t = $('#stateId').DataTable(
			{
				...
				"ajax" : {
					"url" : "paginated?stateName=" + stateName + "&countryName=" + countryName+ "&countryId=" + countryId  + "",
...
var baseUrl = '/admin/pvt/masters/state';
...
function applyReadOnlyProp() {
	$('#stateNameId').prop("readonly", true);
...
function removeReadOnlyProp() {
	$('#stateNameId').prop("readonly", false);
...
function showBusinessEerror(fieldErrMsgMap) {
	$('#stateNameId_err').html(fieldErrMsgMap.stateName);
	$('#countryNameId_err').html(fieldErrMsgMap.countryId);	
}
