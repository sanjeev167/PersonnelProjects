...
$(document).ready(function() {
	...
	$(document).on("click", "#searchFormButtonId", function(e) {
		...
		searchAndReloadGrid(departmentId,moduleNameId,pageNameId);
	});
...
$('#pageNameId').on('change', function(e) {
	cleanAllMsg();	
	var pageId=$('#pageNameId').val();	
	loadPageBaseUrl(pageId);	
});
//Reload Grid.
function reloadGrid() {
...
function searchAndReloadGrid(departmentNameId,moduleNameId,pageNameId) {
	clearCheckBoxSelection();
	var table = $('#opId').DataTable();
	table.ajax.url(
			"paginated?departmentNameId=" + departmentNameId + "&moduleNameId=" +moduleNameId
			+ "&pageNameId=" +pageNameId).load();
...
var t;
function loadGrid(departmentNameId,moduleNameId,pageNameId) {
	t = $('#opId').DataTable(
			{				
...
function loadRecordInForm(response) {
	...
	$('#pageNameId').empty()
    .append('<option selected="selected" value="">-Select-</option>');
	loadPageCombo(response.moduleNameId,response.pageNameId);
	
	
	$('#baseurlId').val(response.baseurl);
	
	$('#opNameId').val(response.opName);
	
	$('#opUrlId').val(response.opUrl);
...
function showBusinessEerror(fieldErrMsgMap) {	
	...
	$('#pageNameId_err').html(fieldErrMsgMap.pageNameId);	
	$('#baseurl_err').html(fieldErrMsgMap.baseurl);
	
	$('#opName_err').html(fieldErrMsgMap.opName);
	
	$('#opUrl_err').html(fieldErrMsgMap.opUrl);
...
function loadPageBaseUrl(pageId){
	
	/* stop form from submitting normally */
	event.preventDefault();	
	method = 'GET';
	
	url = "/admin/pvt/sec/env/page" + "/getRecord" + "?id=" + pageId;
	// alert("posting url = "+url);
	$.ajax({
		type : method,
		url : url,
		...
		success : function(response) {
			...
			if(response.status){
				//alert(response.formObject.countryNameId);				
				$("#baseurlId").val(response.formObject.baseurl);
			}else{
				showBusinessEerror(response.fieldErrMsgMap);
			}
			// Now load the page with response
