...
$(document).ready(function() {
	...
	$(document).on("click", "#searchFormButtonId", function(e) {
		var countryId = $('#countryNameId').val();
		var stateNameId = $('#stateNameId').val();			
		searchAndReloadGrid(countryId,stateNameId);
	});
...
$('#countryNameId').on('change', function(e) {
	...
	$('#stateNameId').empty().append('<option  value="">-Select-</option>');	
	loadStateCombo($('#countryNameId').val(),"");
});
...
function reloadGrid() {
	...
	window.location.replace("/admin/pvt/masters/city/");
...
function searchAndReloadGrid(countryNameId,stateNameId,cityName) {
	clearCheckBoxSelection();
	var table = $('#cityId').DataTable();
	table.ajax.url(
			"paginated?countryNameId=" + countryNameId + "&stateNameId=" +stateNameId).load();
...
function GetSelectedRowID() {
	...
	$(".chkIndvRow").each(function() {
		if ($(this).is(':checked')) {
			...
			checkedRowNames.push(selectedRows[0].countryName +" => "+
					             selectedRows[0].stateName +" => "+" => "
					             +selectedRows[0].cityName + "</br>");
...
var t;
function loadGrid(countryNameId,stateNameId) {
	t = $('#cityId').DataTable(
			{
				"responsive": true,
				...
				"ajax" : {
					...
					"url" : "paginated?countryNameId=" + countryNameId + "&stateNameId=" + stateNameId + "",
...
var baseUrl = '/admin/pvt/masters/city';
...
function loadRecordInForm(response) {
	$('#cityRecordId').val(response.id);	
	$('#countryNameId').val(response.countryNameId);
	//This will clean the state combo of state and show without select in the combo box	
	$('#stateNameId').empty().append('<option  value="">-Select-</option>');	
	selectedId=response.stateNameId;
	loadStateCombo(response.countryNameId,selectedId);
	$('#stateNameId').val(response.stateNameId);
	$('#cityNameId').val(response.cityName);
...
function showBusinessEerror(fieldErrMsgMap) {	
	$('#countryNameId_err').html(fieldErrMsgMap.countryNameId);	
	$('#stateNameId_err').html(fieldErrMsgMap.stateNameId);
	$('#cityNameId_err').html(fieldErrMsgMap.cityName);
}
...
function saveAndUpdateRecord() { 
	...
	var id = $('#cityRecordId').val();	
	var countryNameId = $('#countryNameId').val();	
	var stateNameId = $('#stateNameId').val();
...
function loadCountryCombo(){	
	/* stop form from submitting normally */	
	method = 'GET';
	url = "/admin/pvt/masters/country" + "/list";	
	$
	.ajax({
		type : method,
		url : url,
		...
		success : function(response) {			
			if(response.status){
				var ele = document.getElementById('countryNameId');	       
...
function loadStateCombo(id,selectedId){	
	...
	/* stop form from submitting normally */	
	method = 'GET';
	url = "/admin/pvt/masters/state" + "/list?id=" + id;		
	$
	.ajax({
		type : method,
		url : url,
