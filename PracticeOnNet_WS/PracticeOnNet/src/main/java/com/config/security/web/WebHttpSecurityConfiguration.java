...
public class WebHttpSecurityConfiguration {	
    ...
    public  void prepareAntMatcherForAdministrativeUserChannel(HttpSecurity http,UrlAndRoleMatcherService urlAndRoleMatcherService) throws Exception {
    	...
    	roleArr.add("DEO_MASTER");
    	//roleArr.add("SUPER_ADMIN");
        http.csrf().disable();
        Object[] roleObjArr=roleArr.toArray();
        String x[]= Arrays.copyOf(roleObjArr, roleObjArr.length,String[].class); 
        System.out.println("Sanjeev = "+x.toString());
        http.authorizeRequests()
        //.antMatchers("/admin/pvt/**").hasAnyRole("SUPER_ADMIN");
        .antMatchers("/admin/pvt/masters/country/").hasAnyRole("DEO_MASTER");
        http.authorizeRequests().antMatchers("/admin/pvt/postLogin").hasAnyRole(x)
        .antMatchers("/admin/pvt/db").hasAnyRole("DEO_MASTER");
